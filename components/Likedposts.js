...
function Savedposts() {
    ...
    useEffect(() => 
        onSnapshot(collection(db, "users", session.user.email, "likedposts"), (snapshot) => {
    ...
    <div>
        ...
        </div>
        {
            ...
            <>
                {
                    posts.map((post) => {
                        if(likedPosts.findIndex((likedpost) => (likedpost.id === post.id)) !== -1){
                            return (
                                <Post
                                key={post.id}
                                postId={post.id}
